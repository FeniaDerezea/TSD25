...
parameter.names <- c( 'theta','rho','sd','sumtpr','sumfpr','spec','Theta','delta','beta','predtpr','predfpf','Lambda','vartheta','varalpha')
...
sensres<-quantile(model1.sim$sims.list$sumtpr, c(0.5,0.025,0.975))
...
Summary_Se<-matrix(model1.sim$sims.list$sumtpr,ncol=1)
...
sens<-median(model1.sim$sims.list$sumtpr)
fpf<-median(model1.sim$sims.list$sumfpr)
...
nsims<-length(model1.sim$sims.list$spec)
sumtpr<-model1.sim$sims.list$sumtpr
